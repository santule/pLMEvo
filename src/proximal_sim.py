...
def closest_seq(lg_1d,k,s):
    ...
    return [s,list(k_nearest_idx),mean_d]
...
def proximal_sim_analysis(fasta_aln_file,model_type,k):
    ...
    rand_seqs = list(np.random.choice(len(seq_ref_dict), size = 5,replace=False))
    print(f"Randomly selected {len(rand_seqs)} random sequences.")
    
    knn_lg  = get_knn_sequences(k,lg_mat_np,rand_seqs)
    knn_plm = get_knn_sequences(k,plm_mat_np,rand_seqs)

    knn_lg_df = pd.DataFrame(knn_lg,columns=['ref_seq_idx','lg_knn_seq','lg_mean_dist'])
    knn_plm_df = pd.DataFrame(knn_plm,columns=['ref_seq_idx','plm_knn_seq','plm_mean_dist'])
...
if __name__ == "__main__":
    ...
    df_final = proximal_sim_analysis(fasta_aln_file,model_type,k)
    print(f"Proximal homolog similarity analysis complete")
    print(df_final.head())